...
local playerVehPos
local playerVehDir
local currentValueRight
local hack_dt

local function takeStep(targetValue, target, xdt, currentValue, clampMin, clampMax, smoothness)
  
    --Clamp the targetValue according to the min and max which allows for an offset to occur.
    local step = (targetValue - currentValue) * xdt * smoothness --5.2
...
function SetDirVec(vector)
    ...
    currentValueRight = takeStep(UpdateSpotlight(posVec, dirVec, playerVehPos, playerVehDir)%360, "testvalue", hack_dt, currentValueRight, 0, 360, 2.5)
    electrics.values.spotSin = math.sin(math.rad(currentValueRight))*0.6
    electrics.values.spotCos = math.cos(math.rad(currentValueRight))*0.6
